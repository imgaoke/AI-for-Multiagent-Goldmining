...
{ include("big3_search_unvisited.asl") }// plans for finding gold
...
{ include("big3_allocation_protocol.asl") }  // plans for the gold allocation protocol
...
+!inform_gsize_to_leader(S) : .my_name(miner1)
   <- ?depot(S,DX,DY);
      .send(big3_leader,tell,depot(S,DX,DY));
      //.print("leader name is :", leader, " ");
      ?gsize(S,W,H);
      .send(big3_leader,tell,gsize(S,W,H)).
